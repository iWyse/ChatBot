(() => {
    ...
    function printDay(todayHours) {
        if (todayHours >= 6 && todayHours < 12) return "утра";
        else if (todayHours >= 12 && todayHours < 18) return "дня";
        else if (todayHours >= 18 && todayHours <= 23) return "вечера";
        else return "ночи";
    }

    function spendMyTime(todayHours) {
        if (todayHours >= 6 && todayHours < 12) return "утро";
        else if (todayHours >= 12 && todayHours < 18) return "день";
        else if (todayHours >= 18 && todayHours <= 23) return "вечер";
        else return "ночь";
    }
    let logtime = document.querySelector(".logtime");
    let todayDate = new Date;
    ...
    sendMessage.addEventListener("submit", (function(e) {
        let presentMessage = inputMessage.value;
        if ("" === inputMessage.value) e.preventDefault();
        else {
            let logMessage;
            let sendDate = new Date;
            let sendMinuts = (sendDate.getMinutes() < 10 ? "0" : "") + sendDate.getMinutes();
            ...
            setTimeout((() => {
                let message;
                if ("привет, смотрю сериал" == presentMessage.toLowerCase()) message = `Здорово, я вот жду 3 сезон сериала "видеть"`;
                else if ("как твои дела?" == presentMessage.toLowerCase()) message = "Недавно ходили на море, тебе нравится море?";
                else if ("нравится море, люблю плавать" == presentMessage.toLowerCase()) message = "Вода щас очень теплая, людей правда много";
                else if ("не фанат ходить на море" == presentMessage.toLowerCase()) message = "А мне нравится";
                else if ("что у тебя нового?" == presentMessage.toLowerCase()) message = "Неделю назад приобрел машину";
                else if ("что за машина?" == presentMessage.toLowerCase()) message = "Toyota Soarer, люблю быстро ездить";
                else if ("главное ездить с умом" == presentMessage.toLowerCase()) message = "Обязательно";
                else message = "Прости, появились срочные дела. До встречи!";
